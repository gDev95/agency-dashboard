...
import { ArtistAdvancedInformation, ArtistBasicInformation, Label, Setup } from "../artist.model";

export const validateBasicInformation = (values: ArtistBasicInformation): FormErrors<ArtistBasicInformation> => {
...
export const validateAdvancedInformation = (values: ArtistAdvancedInformation) => {
    const errors: any = {};
...
function validateLabels(labels: Label[]) {
    ...
    if (invalidLabels.length > 0) {
        return {
            _error: "At least one or more Labels where invalid: " + invalidLabels.toString(),
        };
    }
}
...
function validateSetup(setup: Setup) {
    ...
    if (invalidEquipment.length > 0) {
        return {
            equipment: {
                _error: "At least one or more Hospitality where invalid: " + invalidEquipment.toString(),
            },
        };
    }
    ...
    if (isStringEmpty(setup.equipmentImageUrl)) {
        return {
            equipmentImageUrl: "Missing image of equipment, please upload a photo of your setup",
        };
    }
}
...
function validateHospitality(hospitality: string[]) {
    ...
    if (invalidItems.length > 0) {
        return {
            _error: "At least one or more Hospitality where invalid: " + invalidItems.toString(),
        };
    }
}
